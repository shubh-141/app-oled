...
IMAGE_FILE = "/root/testdir/images/embedos_logo.png"
...
font2 = ImageFont.truetype("/root/testdir/fonts/Volter__28Goldfish_29.ttf", 12)
font3 = ImageFont.truetype("/root/testdir/fonts/Exo_Regular400.ttf", 12)
font4 = ImageFont.truetype("/root/testdir/fonts/Red_Alert.ttf", 12)
...
def show_logo(device):
    with canvas(device) as draw:
        ...
        draw.text(
            (10, logo_y_offset + 30), "      info@embedos.io  ", font=font4, fill=255
        )
        draw.text(
            (10, logo_y_offset + 42), "      www.embedos.io  ", font=font4, fill=255
        )
    ...
    with canvas(device) as draw:
        ...
        for i in range(12):
            draw.text(
                (
                    OLED_X / 2
                    + NUMBER_R * math.cos(((i + 1) * 30 - 90) * 2 * 3.14 / 360),
                    OLED_Y / 2
                    + NUMBER_R * math.sin(((i + 1) * 30 - 90) * 2 * 3.14 / 360)
                    - Y_OFFSET,
                ),
                str(i + 1),
                fill=255,
        ...
        draw.line(
            (
                OLED_X / 2,
                OLED_Y / 2,
                OLED_X / 2 + HOUR_HAND_LENGTH * math.cos(hour_angle),
                OLED_Y / 2 + HOUR_HAND_LENGTH * math.sin(hour_angle),
            ),
            fill=255,
        )
        draw.line(
            (
                OLED_X / 2,
                OLED_Y / 2,
                OLED_X / 2 + MINUTE_HAND_LENGTH * math.cos(minute_angle),
                OLED_Y / 2 + MINUTE_HAND_LENGTH * math.sin(minute_angle),
            ),
            fill=255,
        )
...
def device_status():
    ...
    with canvas(device) as draw:
        y_coordinate = 0
        draw.text(
            (0, y_coordinate), "- - - - DEVICE DETAILS - - - - ", font=font4, fill=255
        )
        y_coordinate = y_coordinate + 10

        try:
            draw.text(
                (1, y_coordinate + 10), "TIME : " + device_time, font=font4, fill=255
            )
